...
getwd()
```
#[1] "/local-scratch/localhome/aka259/lab-1-AmritpalKa"
```{bash, engine.opts='-l'}
pwd
```
...
*It overwrites the data existing in the output file.
head -n 25 /local-scratch/course_files/MBB243/Sneddon_genotypes.txt >> ~/MBB243/genotype_head.txt
...
*the result says that there is no such folder found in MBB 243 with the name out.
...
*the data will keep on appending the number of times we run the code chunk given below on the terminal.
...
*#install.packages("reticulate")
> library(reticulate)
> use_python("~/miniconda3/bin/python")* This is the result of the above given command.
...
* In both the cases the same error message appears.
...
```
* Warning message appeared when I ran it on console for the true value as well as false.
...
```
* So when the values were set either to true or false, the same error message was 
